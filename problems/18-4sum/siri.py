        ...
        for i in range(len(nums)):
            ...
            if key in num_3_sum:
                for item in num_3_sum[key]:
                    if i in item['index_list']:
                        continue
                    if item['values'][0] in marked or item['values'][1] in marked or item['values'][2] in marked:
                        continue
                    result.append(
                        [item['values'][0], item['values'][1], item['values'][2], nums[i]])
            marked[nums[i]] = True
